
function getQueryParam(param) {
    const urlParams = new URLSearchParams(window.location.search);
    return urlParams.get(param);
}

// Render Muscle Grid
function renderMuscleList() {
    const container = document.getElementById("muscle-list");
    if (!container) return;

    muscles.forEach(item => {
        const card = document.createElement("a");
        card.href = `workouts.html?muscle=${item.id}`;
        card.className = "card";
        card.innerHTML = `
            <div class="card-icon"><i class="${item.icon}"></i></div>
            <h2>${item.name}</h2>
            <div class="card-arrow"><i class="fa-solid fa-arrow-right"></i></div>
        `;
        container.appendChild(card);
    });
}

// Render Equipment Grid
function renderEquipmentList() {
    const container = document.getElementById("equipment-list");
    if (!container) return;

    equipments.forEach(item => {
        const card = document.createElement("a");
        card.href = `workouts.html?equipment=${item.id}`;
        card.className = "card";
        card.innerHTML = `
            <div class="card-icon"><i class="${item.icon}"></i></div>
            <h2>${item.name}</h2>
            <div class="card-arrow"><i class="fa-solid fa-arrow-right"></i></div>
        `;
        container.appendChild(card);
    });
}

// Render Workouts
function renderWorkouts() {
    const container = document.getElementById("workout-list");
    const title = document.getElementById("workout-title");
    if (!container) return;

    const muscleQuery = getQueryParam("muscle");
    const equipmentQuery = getQueryParam("equipment");

    let filteredWorkouts = [];

    if (muscleQuery) {
        title.innerText = `${muscleQuery.toUpperCase()} WORKOUTS`;
        filteredWorkouts = workoutDB.filter(w => w.muscle === muscleQuery);
    } else if (equipmentQuery) {
        title.innerText = `${equipmentQuery.toUpperCase()} WORKOUTS`;
        filteredWorkouts = workoutDB.filter(w => w.equipment === equipmentQuery);
    } else {
        title.innerText = "ALL WORKOUTS";
        filteredWorkouts = workoutDB;
    }

    if (filteredWorkouts.length === 0) {
        container.innerHTML = `<div style="text-align:center; padding:4rem; background:#fff; border:1px solid #e4e4e7; width:100%;">
            <i class="fa-solid fa-circle-exclamation" style="font-size:3rem; color:#d4d4d8; margin-bottom:1rem;"></i>
            <h3>NO WORKOUTS FOUND</h3>
            <p style="color:#71717a;">We are currently updating our database for this selection.</p>
        </div>`;
        return;
    }

    filteredWorkouts.forEach((workout, index) => {
        const card = document.createElement("div");
        card.className = "workout-card";
        
        // Staggered animation delay
        card.style.animationDelay = `${index * 0.15}s`;
        card.classList.add("fade-in");

        const stepsHtml = workout.steps.map(step => `<li>${step}</li>`).join('');
        
        let mediaHtml = '';
        if(workout.is_video) {
            mediaHtml = `
                <video autoplay loop muted playsinline>
                    <source src="${workout.media_url}" type="video/mp4">
                </video>`;
        } else {
            // Using image tag for GIF or static placeholder
            mediaHtml = `<img src="${workout.media_url}" alt="${workout.name} demonstration">`;
        }

        // Get nice names for tags
        const muscleName = muscles.find(m => m.id === workout.muscle)?.name || workout.muscle;
        const equipName = equipments.find(e => e.id === workout.equipment)?.name || workout.equipment;

        card.innerHTML = `
            <div class="video-container">
                ${mediaHtml}
            </div>
            <div class="workout-details">
                <h3>${workout.name}</h3>
                <div class="tags">
                    <span class="tag"><i class="fa-solid fa-child-reaching"></i> ${muscleName}</span>
                    <span class="tag"><i class="fa-solid fa-dumbbell"></i> ${equipName}</span>
                </div>
                <h4>Execution:</h4>
                <ol class="steps">
                    ${stepsHtml}
                </ol>
            </div>
        `;
        container.appendChild(card);
    });
}
